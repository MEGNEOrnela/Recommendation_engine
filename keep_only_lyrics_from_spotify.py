...
for author, lyrics in data.items():
    for lyric in lyrics:
        name = lyric["title"].lower()
        name = f"{name.strip()}"
        # print()
        if name in list(spotify_tracks.keys()):
            data_filtered[author].append(
